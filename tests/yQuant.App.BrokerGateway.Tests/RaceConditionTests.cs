...
public class RaceConditionTests
{
    ...
    public async Task TestInitialize()
    {
        ...
        {
            Assert.Inconclusive("Redis server is not available. Please start Redis to run these tests.");
        }
    }
    ...
    public async Task UpdatePosition_StressTest_100ConcurrentOrders()
    {
        ...
        if (posJson.HasValue)
        {
            var position = JsonSerializer.Deserialize<Position>(posJson.ToString());
            Assert.IsNotNull(position);
            Assert.IsTrue(position.Qty > 0, "If position exists, quantity should be positive");
            Assert.IsTrue(position.AvgPrice > 0, "If position exists, average price should be positive");
        }
    }
